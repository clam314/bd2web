...
- 2026-07-16：**#17 奇迹紫罗兰·帕莱特(dating17, char004202) 热区/互动流修复**，对照巴哈攻略
  (`forum.gamer.com.tw/C.php?bsn=76207&snA=9954`)正文与两张阶段示意图，再与当前 prefab/Spine
  逐点核验。外部 JSON 的 62 个热区 `source` 与 skel 内同名骨骼 **62/62 精确匹配**；此前网页
  没有对 dating17 启用骨骼跟随，组合后续点仍按 `x≈-12000` 的画外停车坐标投影。现将其加入
  `BONE_FOLLOW_HOTZONE_IDS`，并按游戏 `SetSpineAnimationExternal(track=1)` 路径加入
  `OVERLAY_TRACK_INTERACTION_IDS`；同时删掉旧的 `1_26` 手写 160×160 框，恢复当前 prefab 的
  400×400 真值。Chrome 本地回归确认三条攻略链均可连续点中：**胸部 `1_4→1_5`、腋下
  `2_2→2_3`、膝盖 `2_18→脚底 2_19 / 脚趾 2_20`**，控制台 0 error。攻略“连续抚摸(狂点)”
  对应数据中的 `1_7/2_20`，两点均明确为 `ClickMaxCount=7`、1 秒重置，未擅改次数。另补阶段、
  画笔及上述已确认点位的中文标签；未确认点继续保留编号。
- 2026-07-16：**#14 享乐主义·黎维塔(dating14, char003892) 热区/互动流修复**，对照巴哈攻略
  (`forum.gamer.com.tw/C.php?bsn=76207&snA=8714`)与当前 prefab/Spine 逐项核验。此前 128 个热区
  全按 prefab 静态坐标投影，组合后续点大多停在 `x≈-11400` 的画外停车位，所以只剩一批巨大、
  重叠的基础框，动画中出现的后续点无法点击。审计确认外部 JSON 的 128 个 `source` 与 skel 内
  同名骨骼 **128/128 精确匹配**，因此将 dating14 加入 `BONE_FOLLOW_HOTZONE_IDS`；浏览器实测
  一阶段画面左胸 2 连击后 `1_9/1_11` 随动画进入可点击集，二阶段同点后 `2_28/2_12/2_14`
  进入可点击集，点待机可恢复初始 19 点。互动动画按游戏 `SetSpineAnimationExternal(track=1)`
  路径加入 `OVERLAY_TRACK_INTERACTION_IDS`，避免替换 idle 轨造成姿势组闪回；符咒 100% 的
  `motion1_23 -> stage2` 回归通过。**“2 连击”没有修改**：prefab 中 96 个计量组合点明确为
  `ClickMaxCount=2`、`ContinuousClickResetTime=1`，攻略的一阶段符咒也明确是 50%/100% 两档，
  不是漏抽。另将 tool19/20 按实际 `skin2_on/skin1_off` 与画面对比改名为“触手显示/触手隐藏”，
  补齐两阶段、基础身体/触手与符咒热区的攻略语义标签；未确认的组合后续点保留编号，未猜测命名。
- 2026-07-16：**#13 兔女郎·提尔(dating13, char004102) 交互还原**,对照 PTT 图文攻略
  (`disp.cc/ptt/C_Chat/1fLTAE1j`)+ 巴哈(`C.php?bsn=76207&snA=8444`)。用户报"柴火扔不进火堆":
  52 点全有同名骨骼但未开骨骼跟随,火炉侧隐藏点停在 ±10000 静态停车位永远点不到。修法同
  dating12 三件套:`BONE_FOLLOW_HOTZONE_IDS`+`OVERLAY_TRACK_INTERACTION_IDS`(idle3 会把
  柴火/闭腿/垂臂等 29 个 setup 可见 slot 置空,track0 淡入会闪回)+`MERGED_PREFAB_ACTION_IDS`。
  骨骼揭示链(浏览器采样证实):**一阶段柴火 = 顺序三根**:`1_5`(柴1,常驻)→`mix1_5_1` 搬入
  `1_6`(丢进火炉)→`1_7`(柴2)→`1_8`→`1_9`(柴3)→`1_10`(丢入=motion1_10 热到踢毯)→阶段2;
  阶段2 `2_9` 俄罗斯娃娃(motion2_9 醒来)→阶段3。三阶段:`3_16` 胯下扣子→`3_17` 连点
  (mix3_17_1..9,第9下出 `3_18` 哭)、`3_3` 肚子→`3_4/5/6` 三种捏法(各自 randomMix)、
  `3_19` 左手连点→`3_20` 娃娃脚夹→`3_21` 连点4→`3_22`。道具 15=蛋糕(3点)/16=狗狗(4点)。
  **二阶段特例(与骨骼停车不同的第二种隐藏机制)**:`2_5~2_8` 点名带 `_1hidden_`,骨骼在
  idle2 下不停车,是 prefab 程序隐藏(攻略:长按兔子玩偶 `2_4` 震动后"腿打开一段时间"可摸胸)。
  近似:`2_4_0` drag setFlag `teel13BunnyBuzzed`,`2_5~2_8` visibleWhen 解锁(不限时,同 dating4
  flag 先例);配套给 `playDragSuccess`/`playDragRelease` 补了 setFlag 应用(普通拖拽松手也算,
  只对带 setFlag 的 action 生效)。gyro 的 `mix*_0_1` 同 dating12 在 skel 里不存在,未接。

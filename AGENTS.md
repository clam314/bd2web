...
### ✅ dating11 伊柯利普斯"人物重叠"修复(2026-07-09,含通用机制结论)

**现象**:一阶段"触摸2"(点位 1_1_0,`mix1_1_1`)播放时画面出现两个身体叠在一起;实际
所有 touch/gyro/菜单 drag 都会重叠,不止触摸2。

**根因(浏览器 slot 级分析确认)**:dating11 的 skel 是**多姿势组共存**——ID1(阶段1/2 远景跪姿)、
ID3(阶段3 近景)、ID4(阶段4)三套部件同在一个 skeleton 里。**setup pose 里 ID3 那组是可见的
(112 个 slot)**。idle 每帧把非当前组附件 key 成 null(如 `idle1` 把全部 ID3 slot key 成 NULL),
mix/motion 只 key 自己那组。spine AnimationState 的固有行为是:**track 0 上切动画时,旧动画
key 过而新动画没 key 的 slot 会被回退到 setup pose**(实测 mixDuration 0 和 0.2 都一样)。
所以把 `mix1_1_1` 直接播在 track 0,ID3 整组立刻从 setup 冒出来 => 双层人物。
当天早些的 `0d4ca0c`(hardCut + `setSlotsToSetupPose` + `mix1_1_1` 起始偏移 0.75s)是错误方向,
setup 重置反而把 ID3 一直摆在画面上,已全部撤销。

**正确修法(与真机一致,dating.html 已实现)**:真机逆向早已确认互动 mix 走
`SetSpineAnimationExternal(name, track=1, loop=false)`(见 `playDragBegin` 注释)——**idle 常驻
track 0 持续压住非当前组附件,互动动画全部叠在 track 1**。改动:
- `shouldUseTouchOverlayTrack` 对 dating11 全量互动(touch/gyro/菜单 drag)返回 true(dating6
  原有的 indexed-only 条件不变);
- overlay 分支:带 `nextStage` 的序列**不加** `addEmptyAnimation` 收尾(保持末帧,等
  `enterStage -> playIdle` 清 track 1 接管),无推进的照旧 0.12s 淡出回待机;
- track 1 的 SFX/动作语音走 TrackEntry 自带 `listener`(`overlayAudioListener`,entry 回池时
  runtime 自动清 listener,不污染全局 start 监听);
- `playIdle` 清 track 1 的分支从 dating6-only 扩到 dating11。

**验证过**(本地逐帧 + slot 计数):阶段1 触摸/拖拽、1→2(touch 推进)、2→3、3→2、2→4(gyro,
需选"道具14"才显示)、阶段3/4 触摸,全程无异组泄漏;dating6 indexed 连点回归正常。

**通用结论(以后别再踩)**:凡是"多姿势组"skel(点互动会切近景/远景的角色),**mix/motion 绝不能
播在 track 0**,否则 spine 会把未 key 的 slot 回退 setup 造成重叠/穿帮;一律 idle track 0 +
互动 track 1。其他角色若再报"重叠",先按本节方法做 slot 前缀分析(见 git log 本次 commit)。

### ✅ dating6 威廉明娜同类问题一次修完(2026-07-15,overlay 架构统一)

用户回报 dating6/11 仍有"一闪而过的重叠"、dating6 拖拽时"两双手"、阶段3热区乱飞、阶段切换滞后。
逐个复现定位后确认全部同根:**track 0 上任何 mix/motion 切换的交叉淡入窗口(0.2-0.25s)里,
spine 会把旧动画 key 过、新动画没 key 的 slot 回退到 setup pose**——dating6 也是多姿势组
skel(ID1/ID2/ID3/ID5/ID6 + `hands`=玩家的手 18 slot;mix3_3_1 用"替代手"slot `ID1_hand(R)_3/4`
替换原手 `_1/_2`,闪的瞬间新旧两只手同屏="两双手")。实测干净路径:track1 内 A→B 切换、
track1 empty 淡出、idle→idle 换阶段(idle 附件全覆盖)都无闪;**只有 track0 播局部覆盖动画闪**。

统一修法(`OVERLAY_TRACK_INTERACTION_IDS` = {dating6, dating11},一套代码):
1. `shouldUseTouchOverlayTrack` 对集合内角色全量互动(touch/gyro/菜单 drag)返回 true;
   holdPose 分支(dating6 阶段6连锁冻结)优先级不变。
2. 公共入口 `playOverlaySequence(sequence, targetStage)`:track1 叠播 + entry 自带 listener 补
   SFX/动作语音;**时长≈0 的单帧姿势(dating6 连点链 mix2_1_1 等 duration=0)不加 empty 收尾**
   (原来被 0.12s empty 瞬间抹掉=点了没反应+骨骼来回跳带着热区乱飞),链收尾 `mix*_end`(1.67s)
   或超时重置(scheduleTouchStop,indexed 现在带 `_touchProgressKey`)才淡出。
3. 带 `nextStage` 的序列:末条 entry 的 complete listener 里 `setTimeout(0)` 立即 enterStage
   (修"滞后";advanceTimer 只兜底)。setTimeout 是必须的——enterStage→playIdle 会 clearTrack(1)
   清掉正在派发事件的 entry。
4. `playDragSuccess`/`advanceStage`/`scheduleTouchStop` 的 stopMix 对集合内角色也走
   `playOverlaySequence`(拖拽成功 motion 原来在 track0 = "两双手一闪"主现场)。
5. 阶段推进期间 `hotzonesSuppressed` 隐藏热区(过渡 motion 把跟随骨骼甩满屏,旧阶段热区跟着飞),
   enterStage/playIdle 恢复并重渲染。

**验证过**(同步逐帧+真实指针事件):dating6 菜单拖拽无闪(extras 平滑 0→2)、5连点姿势保持/收尾/
track1 清空、2→3 菜单推进即时+热区抑制恢复、3_3_0 指针拖到目的地→motion3_3(track1)→阶段4、
阶段6连锁 mix6_5_1 冻结+后续热区 6_6_0/6_21_0 出现;dating11 触摸 ID3 逐帧恒 0、1→2 推进正常。
⚠️ 预览面板被遮挡时 canvas rect=0,热区投影全 null、rAF 冻结——测热区必须先让面板可见(截一张图)。

**2026-07-15 补:dating11 热区改骨骼跟随(进二阶段的隐藏点链)**。用户报"触摸10 的热区没有,
进不了二阶段"。审计发现 dating11 全部 point 热区都有同名骨骼(仅 InteractionZone 无),攻略链
是骨骼进出画面揭示隐藏点:**mix1_3_1(触摸4)把 point1_5_touch 搬进画面 0.03~1.67s、
mix1_5_1(触摸6)把 point1_9_touch 搬进画面 0.03~2s,窗口内点中 1_9 才触发 motion1_9 进二阶段**;
阶段3/4 的 3_6~3_16、4_7~4_19 等成批停车点同理。静态热区坐标永远停在 ±12000 停车位,永远点不到。
修法:`BONE_FOLLOW_HOTZONE_IDS` 加 illust_dating11 + 删掉 HOTZONE_OVERRIDES 里手写的 1_9_0
静态框(坐标本来就错,外部 JSON 400×400 才是 prefab 真值)。已实测完整链:热区点 1_3→1_5 现身→
点 1_5→1_9 现身→点 1_9→阶段2(热区 2_1_14/2_2_14 正常)。菜单按钮直点 触摸10 也仍然可用。

**后续修订(同日,用户反馈 dating6/11 仍有"一闪而过"的重叠)**:逐帧 + 轨道快照定位到第二个根因,
与淡出混合无关——**track 0 的 idle 是 loop 动画,每循环一圈就触发一次 spine `complete` 事件**;
overlay 互动进行中 `restoringIdle=false`,complete 监听器便在循环边界重进 `playIdle()`,而
`playIdle` 里的 `setSlotsToSetupPose()` 是在本帧 apply **之后**(queue.drain 回调里)执行的,
于是这一帧带着 setup pose(dating11=ID3 整组可见)被渲染出去 => 单帧重叠闪烁。dating6 闪烁同源
(它的 overlay 分支早就存在这个问题)。修法两条,都在 dating.html:
1. `complete` 监听器加 `track.loop` 早退——循环待机的每圈 complete 不该触发"回待机";
2. `playIdle` 在 `setAnimation` 后立刻同步 `animationState.apply(skeleton)` 一次,保证
   setSlotsToSetupPose 的 setup 状态永远不会被渲染成一帧(阶段切换等合法 playIdle 同样受益)。
验证:dating11 触摸/拖拽/晃动各 6-8 秒逐帧(含多个 idle 循环边界)、全部跨阶段转场 0 异常帧;
dating6 索引连点后 3 圈 idle 循环 0 闪帧、track 0 entry 不再被替换。
**教训**:排查"闪一帧"类问题,别先猜混合曲线,先抓"哪帧、前后轨道状态"——是事件回调在
apply 之后改了骨架状态。

### ✅ dating12 杰尼斯尊爵交互还原(2026-07-16,骨骼跟随+overlay 两套机制直接套用)

用户报"热区乱套、不分阶段全出来"。根因与修法都无新机制,就是把 dating12 挂进已有两套集合:
- **热区**:87 点全有同名骨骼但没进 `BONE_FOLLOW_HOTZONE_IDS`,静态停车位坐标把隐藏点
  (计量表/节奏圈,停在 ±13000/±50000)全画在画面右侧叠成一堆。加入集合后隐藏链全自动:
  摸头1_22→计量表1_23连点→1_24→节奏圈1_25/27/29/35(失败点 1_26/28/30/31 同位叠置,已
  hidden,网页无缩圈时机判定点中即成功)→motion1_35→阶段2→2_1→二型态;二型态 3_21 冰层
  连点→3_22→3_23 掉冰柱→3_24~32,`mix3_32_1`=23.3s 完整隐藏演出(拖冰柱→两腿中间,末段
  3_43 现身)。流程对照巴哈攻略 C.php?bsn=76207&snA=8291(需本地 Chrome 过防护)。
- **重叠**:也是多姿势组 skel(B=一阶段/C+S=二型态),进 `OVERLAY_TRACK_INTERACTION_IDS`。
- **新增通用修复 `resetFollowPointBones()`**:playIdle 的 clearTrack(1) 掐掉进行中 mix 时,
  被停到停车位的 point* 骨骼永远没人 key 回来(空动画淡出会还原到 setup,但 clearTrack 是硬切),
  23s 演出里换阶段/点待机必现"热区永久消失"。playIdle 里对骨骼跟随角色把全部 point* 骨骼
  `setToSetupPose()`。dating6/9/11 回归通过。
- 道具 17=冰块/18=嘴巴(`TOOL_LABEL_OVERRIDES`);dating12 进 `MERGED_PREFAB_ACTION_IDS`
  (外部 JSON + 前端门禁/hidden 合并);gyro 的 `mix*_0_1` 在 skel 里不存在(真机=倾斜视差,
  对应 idle2_l*/r* 变体),未接、无影响。
